...
# see: http://www.diag.uniroma1.it/oriolo/amr/slides/WheeledMobileRobots5_Slides.pdf
def goal_to_polar_coordinates(x, y, theta, gx, gy, gtheta=None):
    dist = math.sqrt((gx - x) ** 2 + (gy - y) ** 2)
    gamma = angle_diff(math.atan2(gy - y, gx - x), theta)
    if gtheta is not None:
        delta = angle_diff(gamma, theta) + gtheta
        return dist, gamma, delta
    else:
        return dist, gamma, None


def polar_coordinates_position_regulation(dist, gamma, k1=1, k2=3):
    v = k1 * dist * math.cos(gamma)
    w = k2 * gamma
    return v, w


def polar_coordinates_pose_regulation(dist, gamma, delta, k1=1, k2=3, k3=1):
    v = k1 * dist * math.cos(gamma)
    w = k2 * gamma + k1 * (math.sin(gamma) * math.cos(gamma)) / gamma * angle_diff(delta, gamma)
    print(rad2deg(delta))
    return v, w
...
class PlanarEnvironment(EnvironmentBase):
    ...
    def get_agent_observation(self, agent_name: str):
        ...
        dist, gamma, delta = goal_to_polar_coordinates(x, y, theta, gx, gy, gtheta)
        return dist, gamma, delta
...
class UnicycleAgent:
    ...
    def compute_action(self, obs):
        if obs is None:
            ...
            dist, gamma, delta = obs
            return polar_coordinates_pose_regulation(dist, gamma, delta)
...
env.set_agent_goal("unicycle", [2, 3, 0])
env.reset()
